...
async def upsert_channel(
    ...
    stmt = stmt.on_conflict_do_update(
        ...
        set_={'title': title, 'username': username, 'is_tracked': True},
    )
...
async def get_channel_by_tg_id(session: AsyncSession, tg_channel_id: int) -> Channel | None:
    return await session.scalar(select(Channel).where(Channel.tg_channel_id == tg_channel_id))
